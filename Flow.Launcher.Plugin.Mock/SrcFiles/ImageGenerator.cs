...
public static class ImageGenerator {
    ...
    public static BitmapImage CreateImage(string inputImagePath, string outputImagePath, string text) {
        ...
        var fontSize = image.Height / 13; // Set font size to 7.69% of the image height (hehe 69 :3)
        var textMarginBottom = image.Height / 25; // Set text margin bottom to 4% of the image height
        var strokeWidth = fontSize / 5; // Set stroke width to 10% of the font size
    ...
    private static List<string> SplitImageTextIntoLines(string text, SKFont font, SKBitmap image) {
        ...
        foreach (var word in words) {
            ...
            if (!(textWidth > image.Width)) continue;
